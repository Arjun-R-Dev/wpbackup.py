...
SFTP_KEY = '/home/ec2-user/.ssh/id_rsa'
...
def sftp_upload(archive_path):
    ...
    try:
        knownhosts = '/tmp/dummy_knownhosts'
        knowhosts_content = """0.0.0.0 ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAQEApU+isf3wivAlAp5/ \
oQ6mQnA5iVQAlSHKb5+kBw3IavtZTyttQN8Vuo3eKMfAMssA89dF+dxWsDGjeiIhWv/DZlnKrzPL71VK3JfTOUY8\ 
ZT8NMJg3c2k25Sm9p1Nxb0SL4KMClBaB8BX9Qty7ZkCSCMaArArflc5CNqsTM10ZqXat9VWctk5s6NsDMZDup44+q \
Dk9GSrkyIjWYm4Vw8WTo6Bc1nRy7xJriimsfbdEiUcjnBOUDfMQL6d/wklyOJNKShAPMjsup8zx0LmpXRXvpdaQgIIx \
CSw/WMyaNVFwGQW2N0HffEyMhRKwwoR4m5TlXWGX76jAgA6+WQxi885FLw=="""

        with open(knownhosts,'w') as fh:
          fh.write(knowhosts_content)
          fh.close()
  
        opts = pysftp.CnOpts(knownhosts='/tmp/dummy_knownhosts')
        opts.hostkeys = None
        cnopts = pysftp.CnOpts()
        cnopts.hostkeys = None 
        with pysftp.Connection(SFTP_HOST,username=SFTP_USER,private_key=SFTP_KEY,port=int(SFTP_PORT),cnopts=cnopts) as sftp:
